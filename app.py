...
st.set_page_config(page_title="Cloudy&Shiny Index", page_icon="M", layout="wide")
st.markdown(
    """
<style>
...
if data.empty:
    st.error("No sentiment data yet. Run python sentiment_tracker.py first.")
    st.stop()
...
if missing:
    st.error(f"Missing columns in {DATA_FILE}: {', '.join(sorted(missing))}")
    st.stop()

scored_data = data.dropna(subset=["mood_score"]).copy()
if scored_data.empty:
    st.error("No valid mood_score records in sentiment_data.csv.")
    st.stop()
...
components.html(template_html, height=1560, scrolling=True)
